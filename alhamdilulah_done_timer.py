...
#xq=threading.Timer(target=hello,args=())
#xq.start()
halt_pid='False'
...
def gfg():
            ...
            print("running")
            global e5,e6,check_temperature,gfg1,l_cool,list_set,list_stop,halt_timer,e_time1,l_f_hrs,l_f_mins,timer_close,timing_end
...
###########################################RESULT SCREEN###########################################################
    ...
    read_time=0
    def read_every_time():
        global read_time,variablesearch,timing_end,list_names,list_set,check_temperature,l_cool,e5,e6,check_temperature,timer_close,l_cool,list_set,list_stop,vartime,vartoggle,varhour,vartime2
        #print(variablesearch.get())
        today = date.today()
...
##    for row in range(1):
##                a = row
##                Label(f_about.scrollFrame.viewPort, text='', width=150,height=43, borderwidth="0",).grid(row=row, column=0)
    ...
    e_server1 = Text(f_about1.interior) 
...
#############################################tkinter_graph######################
    ...
    def stop_test():
        ...
        global list_set,l_1,l_2,list_stop,halt_timer,gfg1,s11,var2,gk1,set_point_original,var_time,list_names,timing_end
    ...
    only1=1
    def animate(i):
        global halt_pid,l_1,l_2,list_date,rt,list_set,check_temperature,pid_temperature,set_point_original,only1,timing_end
        ...
        if(list_set[-1]=='True'):
            ...
            y=temp()
            l_1.append(y)
            l_2.append(current_time_1)
            if only2==1:
                only2=0
                xq_1=threading.Thread(target=initPID,args=())
                xq_1.start()
            #axc[0].append(y)
            #bxc[0].append(current_time_1)
    ...
    def reading_graph():
        global list_stop,gfg1,l_f_hrs,l_f_mins,var_time
